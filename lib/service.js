...
import drawTabbableOnCanvas from "./canvas-lines";
...
export default class WdioImageComparisonService extends BaseClass {
    ...
    before(capabilities) {
         ...
         */
        browser.addCommand('saveTabbable', function (tag, saveFullPageScreenOptions = {}) {
            browser.execute(drawTabbableOnCanvas);

            const data = saveFullPageScreen({
                    executor: this.execute.bind(browser),
                    screenShot: this.takeScreenshot.bind(browser),
                },
                instanceData,
