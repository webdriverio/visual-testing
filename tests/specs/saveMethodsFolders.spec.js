...
describe('wdio-image-comparison-service save methods folder options', () => {
    ...
    describe('saveFullPageScreen method with folder options', () => {
        it('should set folders using method options', async () => {
            const results = await browser.saveFullPageScreen('saveFullPageFolderOptions', testOptions);
            expect(results.path).toMatch(testOptions.actualFolder.replace('./', ''));
        });

        it('should set folders using default options', async () => {
            const results = await browser.saveFullPageScreen('saveFullPageDefaultOptions', {});
    ...
    describe('saveScreen method with folder options', () => {
        it('should set folders using method options', async () => {
            const results = await browser.saveScreen('saveScreenFolderOptions', testOptions);
            expect(results.path).toMatch(testOptions.actualFolder.replace('./', ''));
        });

        it('should set folders using default options', async () => {
            const results = await browser.saveScreen('saveScreenDefaultOptions', {});
            expect(results.path).toMatch('.tmp');
        });
    });
    ...
    describe('saveElement method with folder options', () => {
        it('should set folders using method options', async () => {
            const results = await browser.saveElement(await $('.uk-button:nth-child(1)'), 'saveElementFolderOptions', testOptions);
            expect(results.path).toMatch(testOptions.actualFolder.replace('./', ''));
        });

        it('should set folders using default options', async () => {
            const results = await browser.saveElement(await $('.uk-button:nth-child(1)'), 'saveElementDefaultOptions', {});
            expect(results.path).toMatch('.tmp');
        });
    });
});
